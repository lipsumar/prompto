...
export default {
  ...
  methods: {
     ...
     */
    zoomRect(rect: any, opts: { scale: number | null } = { scale: null }) {
      ...
      const x =
        -rect.left * scale + ((screen.clientWidth / scale - width) / 2) * scale;
      const y =
        -rect.top * scale +
        ((screen.clientHeight / scale - height) / 2) * scale;
      this.panzoom.zoom(scale);
      this.panzoom.pan({ x, y });
    },
    zoomNode(node: any) {
     ...
     */
    zoomNodes(nodes: any, opts = { padding: 50, scale: null }) {
    ...
    panNode(node: any, opts: any = { offsetX: 0, offsetY: 0 }) {
      // centers node on screen
      const offsetX = opts.offsetX || 0;
      const offsetY = opts.offsetY || 0;
      const zoom = this.panzoom.getZoom();
      const x =
        this.$el.clientWidth / 2 - (node.x + node.width / 2) * zoom + offsetX;
      const y =
        this.$el.clientHeight / 2 - (node.y + node.height / 2) * zoom + offsetY;
      this.panzoom.pan({ x, y });
    },
  },
};
