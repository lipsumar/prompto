...
<template>
  <path class="edge" :d="path" :id="data.id" v-bind="$attrs"> </path>
</template>
...
export default {
  ...
  computed: {
    fromNode: (vm: any) =>
      typeof vm.data.from === "string"
        ? vm.nodes.find((n: any) => n.id === vm.data.from)
        : vm.data.from,
    toNode: (vm: any) =>
      typeof vm.data.to === "string"
        ? vm.nodes.find((n: any) => n.id === vm.data.to)
        : vm.data.to,
    fromAnchor: (vm: any) => vm.parseAnchor(vm.data.fromAnchor, vm.fromNode),
    toAnchor: (vm: any) => vm.parseAnchor(vm.data.toAnchor, vm.toNode),
    ...
    pos() {
      ...
      if (this.fromAnchor && this.fromAnchor.snap) {
        if (this.fromAnchor.snap === "circle") {
          throw new Error("need victor");
          // const radius =
          //   Math.max(this.fromNode.width, this.fromNode.height) / 2;
    ...
    path() {
      ...
      if (
        this.data.type === "hsmooth" ||
        (this.data.type === "smooth" && Math.abs(distX) >= Math.abs(distY))
      ) {
        ...
        this.data.type === "vsmooth" ||
        (this.data.type === "smooth" && Math.abs(distY) > Math.abs(distX))
      ) {
  ...
  methods: {
     ...
     * }
     */
    parseAnchor(anchor: any, node: any) {
