...
export default class Graph {
  ...
  positionNode({
    ...
    node =
      typeof node === "string" ? this.nodes.find((n) => n.id === node) : node;
    parent =
      typeof parent === "string"
        ? this.nodes.find((n) => n.id === parent)
        : parent;
    const pos = util.findPosition(
      node,
      parent,
      dir,
      this.nodes,
  ...
  graphNodes({
    ...
    if (type === "basic" || type === "basic-invert") {
      ...
      const findPos = (node: any, parent: any) => {
        if (visited[node.id]) {
          return;
        }
        const collisions = nodes.filter((n: any) => !!visited[n.id]);
        const pos = util.findPosition(
          node,
          parent,
          dir,
        ...
        node.children.forEach((n: any) => findPos(n, node));
      };
      dag
        .filter((node: any) => !node.parentIds.length)
        .forEach((node: any) => findPos(node, null));
    } else {
  ...
  createEdge(from: any, to: any = false, fields = {}) {
    if (to === false) {
      // support calling with single argument
      // eslint-disable-next-line prefer-rest-params
      fields = arguments[0];
      from = (fields as any).from;
      to = (fields as any).to;
    } else {
  ...
  updateEdge(edge: any, fields: any) {
          ...
          if (otherNode.id === node.id) {
            ...
            if (
              this.edges.find((edge) => {
                return (
                  (edge.to === node.id && edge.from === otherNode.id) ||
